...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [_contentView setupAutoContentSizeWithBottomView:_payBtn bottomMargin:0];
}
...
- (void)countDateSecond{
//    NSDate *nowDate = [NSDate date];
//    NSDateFormatter *dateFomatter = [[NSDateFormatter alloc] init];
//    dateFomatter.dateFormat = @"yyyy-MM-dd HH:mm:ss";
//    
//    // 截止时间字符串格式
//    NSString *expireDateStr = _orderCreateTime;
//    // 当前时间字符串格式
...
- (UIScrollView *)contentView{
    if (!_contentView) {
        ...
        [_contentView addSubview:self.payTableView];
        [_contentView addSubview:self.payBtn];
    }
    return _contentView;
}
