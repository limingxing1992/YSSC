...
- (void)viewDidLoad {
    ...
    self.contentScrollView.contentSize = CGSizeMake(SCREEN_WIDTH, SCREEN_HEIGHT - 10);
    [self.view addSubview:self.topView];
    [self.contentScrollView addSubview:self.secondView];
    [self.contentScrollView addSubview:self.thirdView];
    [self.contentScrollView addSubview:self.roomTypeView];
    [self.contentScrollView addSubview:self.fourthView];
    [self.contentScrollView addSubview:self.fifthView];
    [self.contentScrollView addSubview:self.payTableView];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    .topSpaceToView(self.contentScrollView, 10)
    ...
    switch (_payStatus) {
        case 0:
        {
            ...
            [YSSCShareBusinessManager.hotelManger getHotelAliPayWithDictionary:dict success:^(id object) {
                ...
                if (![aliPay.errorCode isEqualToNumber:@1]) {
                    ...
                    NSLog(@"支付结果返回： %@", aliPay.errorMsg);
                    return ;
                }
               
                [[AlipaySDK defaultService] payOrder:aliPay.data fromScheme:@"wx4bdeb11b1339f074" callback:^(NSDictionary *resultDic) {
                    NSLog(@"callback支付结果返回： %@", resultDic);
                }];
            } failure:^(NSInteger errCode, NSString *errorMsg) {
                [SVProgressHUD showErrorWithStatus:errorMsg];
            }];
        ...
        case 1:
        {
            ...
            if ([WXApi isWXAppInstalled]) {
                ...
                [SVProgressHUD dismiss];
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"抱歉，您尚未安装微信。" delegate:nil cancelButtonTitle:@"知道了" otherButtonTitles:nil, nil];
                [alertView show];
            }
...
- (UIScrollView *)contentScrollView{
    if (_contentScrollView == nil) {
        _contentScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 64, SCREEN_WIDTH, SCREEN_HEIGHT -64-49)];
    }
